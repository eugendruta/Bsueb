$(document).ready(function () {
    ...
    if ($('#' + lbname).val() === null) {
      ...
      for (let i = 0; i < config.default.data.listboxen.length; i++) {
        if (config.default.data.listboxen[i].name === lbname) {
          ...
          //../Listbox?typ=appKonst&table=V_AC_FLSLAGERORTTYP
          UTIL.logger(dialogname + ': initLb(): lbname: ' + lbname + '; url: ' + url);
  ...
  showtable = function showtable(tabelle) {
    ...
    //var url = "http://localhost:8080/WebRWinMvn/Listbox?typ=";  
    var url = globalconfig.bckendurl + config.default.data[tabelle].servlet + "sqlstm=" + sqlstm;
